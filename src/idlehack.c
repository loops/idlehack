 ...
 *     periodically runs "swayidle-inhibit -deactivate" to keep the
 ...
 * org.freedesktop.ScreenSaver.GetActive qdbus org.kde.screensaver  /ScreenSaver
 * org.freedesktop.ScreenSaver.GetActive qdbus org.gnome.ScreenSaver
 * /ScreenSaver org.gnome.ScreenSaver.GetActive
 *
 *
 *
 * TESTING:
...
#define DBUS_SD_METHOD_WHO "idlehack"
#define DBUS_SD_METHOD_WHY "org.freedesktop.ScreenSaver -> systemd"
#define DBUS_SD_METHOD_MODE "delay"

#define DBUS_SD_MATCH             \
  "type='signal',"                \
...
inhibit_head = SLIST_HEAD_INITIALIZER(inhibit_head);
...
static void xscreensaver_command(const char* cmd) {
  char buf[1024];
  int rc;
  sprintf(buf, "swayidle-inhibit %.100s -%.100s",
          (verbose_p ? "-verbose" : "-quiet"), cmd);
...
static int xscreensaver_register_sleep_lock(struct handler_ctx* ctx) {
  return 0;  // SBE - no need to monitor shutdown or sleep
  sd_bus_error error = SD_BUS_ERROR_NULL;
  sd_bus_message* reply = NULL;
  int fd = -1;
  int rc = sd_bus_call_method(
      ctx->system_bus, DBUS_SD_SERVICE_NAME, DBUS_SD_OBJECT_PATH,
      DBUS_SD_INTERFACE, DBUS_SD_METHOD, &error, &reply, DBUS_SD_METHOD_ARGS,
      DBUS_SD_METHOD_WHAT, DBUS_SD_METHOD_WHO, DBUS_SD_METHOD_WHY,
      DBUS_SD_METHOD_MODE);
  if (rc < 0) {
 ...
 */
static int xscreensaver_systemd_handler(sd_bus_message* m, void* arg,
 ...
 */
static int xscreensaver_method_inhibit(sd_bus_message* m, void* arg,
                                       sd_bus_error* ret_error) {
  struct handler_ctx* ctx = arg;
...
/* Called from the vtable when another process sends a request to systemd
   ...
   request.
 */
static int xscreensaver_method_uninhibit(sd_bus_message* m, void* arg,
                                         sd_bus_error* ret_error) {
  struct handler_ctx* ctx = arg;
  uint32_t cookie;
  ...
  SLIST_FOREACH(entry, &inhibit_head, entries) {
    if (entry->cookie == cookie) {
      ...
      free(entry);
      ctx->is_inhibited--;
      if (ctx->is_inhibited < 0) ctx->is_inhibited = 0;
      found = 1;
      break;
    }
  }
...
static pid_t get_bus_name_pid(sd_bus* bus, const char* name) {
  int rc;
  sd_bus_creds* creds;
  pid_t pid;
 ...
 */
static char* process_name(pid_t pid) {
...
static int xscreensaver_systemd_loop(void) {
   ...
   */
  rc = sd_bus_track_new(user_bus, &global_ctx.track, NULL, NULL);
  if (rc < 0) {
  ...
  rc = sd_bus_add_object_vtable(user_bus, NULL, DBUS_FDO_OBJECT_PATH,
                                DBUS_FDO_INTERFACE, xscreensaver_dbus_vtable,
                                &global_ctx);
  if (rc < 0) {
  ...
  rc = sd_bus_add_object_vtable(user_bus, NULL, DBUS_FDO_OBJECT_PATH_2,
                                DBUS_FDO_INTERFACE, xscreensaver_dbus_vtable,
                                &global_ctx);
  if (rc < 0) {
    fprintf(stderr, "%s: vtable registration failed: %s\n", blurb(),
      ...
      if (rc < 0) {
        ...
        if (pid != -1) {
          ...
          if (pname) {
            ...
            fprintf(stderr, "%s: connection failed: \"%s\" in use by pid %lu\n",
  ...
  */
  rc = sd_bus_add_match(system_bus, NULL, DBUS_SD_MATCH,
                        xscreensaver_systemd_handler, &global_ctx);
  if (rc < 0) {
    fprintf(stderr, "%s: add match failed: %s\n", blurb(), strerror(-rc));
    goto FAIL;
  }

  if (verbose_p) fprintf(stderr, "%s: connected\n", blurb());
  ...
  while (1) {
    ...
    /* Prune any entries whose original sender has gone away: this happens
       ...
       same cookie. */
    SLIST_FOREACH(entry, &inhibit_head, entries) {
      if (entry->peer && !sd_bus_track_count_name(ctx->track, entry->peer)) {
        if (verbose_p)
          fprintf(stderr,
                  "%s: peer %s for inhibiting app \"%s\" has died:"
                  " uninhibiting %08X\n",
                  blurb(), entry->peer, entry->appname, entry->cookie);
        SLIST_REMOVE(&inhibit_head, entry, inhibit_entry, entries);
        if (entry->appname) free(entry->appname);
        free(entry->peer);
        free(entry);
      ...
      if (now - last_deactivate_time >= HEARTBEAT_INTERVAL) {
        if (verbose_p) {
          SLIST_FOREACH(entry, &inhibit_head, entries) {
            ...
            fprintf(stderr, "%s: inhibited by \"%s\" since %s", blurb(),
                    entry->appname, ct);
          }
        }
        xscreensaver_command("deactivate");
    ...
    */
    ...
    poll_timeout_msec =
        ((system_timeout_usec < user_timeout_usec ? system_timeout_usec
                                                  : user_timeout_usec) /
         1000);
     ...
     */
    if (ctx->is_inhibited && poll_timeout_msec > HEARTBEAT_INTERVAL * 1000)
      poll_timeout_msec = HEARTBEAT_INTERVAL * 1000;

    if (poll_timeout_msec < 1000) poll_timeout_msec = 1000;

    rc = poll(fds, 2, poll_timeout_msec);
    if (rc < 0) {
...
FAIL:
  ...
  if (system_bus) sd_bus_flush_close_unref(system_bus);

  if (ctx->track) sd_bus_track_unref(ctx->track);

  if (user_bus) sd_bus_flush_close_unref(user_bus);
...
It invokes 'swayidle-inhibit' to tell the xscreensaver daemon to lock\n\
